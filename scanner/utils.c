...
int is_valid_hash_algorithm(const char* algorithm)
{
    ...
    return (strcasecmp(algorithm, "md5") == 0 || strcasecmp(algorithm, "sha1") == 0 || strcasecmp(algorithm, "sha256") == 0 || strcasecmp(algorithm, "sha512") == 0);
}
...
// RISC-V архитектура
    ...
    char* calculate_file_hash(const char* filepath, const char* algorithm)
    {
        ...
        const size_t BUFFER_SIZE = 65536; // 64 KB
        ...
        if (file_size > 100 * 1024 * 1024) { // Файл > 100MB
            ...
            while (remaining > 0 && (bytes_read = fread(buffer, 1, (remaining < (long)BUFFER_SIZE) ? remaining : BUFFER_SIZE, file)) > 0) {
            ...
            if (fseek(file, -CHUNK_SIZE, SEEK_END) != 0) {
                ...
                fprintf(stderr, "Warning: Cannot seek to end of large file, hashing entire file: %s\n", filepath);
                rewind(file);
                total_read = 0;
    ...
    int is_valid_utf8_string(const char* str)
    {
        ...
        while (*bytes) {
            ...
            // 4-байтовый UTF-8 (0xF0-0xF7)
                ...
                if (!bytes[1] || !bytes[2] || !bytes[3] || (bytes[1] & 0xC0) != 0x80 || (bytes[2] & 0xC0) != 0x80 || (bytes[3] & 0xC0) != 0x80)
                    return 0;
                bytes += 4;
            }
            // Некорректный UTF-8
