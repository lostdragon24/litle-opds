...
char *read_file_content(const char *filepath) {
  ...
  // Ограничиваем размер читаемого файла (80 MB для текстовых файлов)500
  long read_size = file_size;
  if (read_size > 500 * 1024 * 1024) {
    read_size = 500 * 1024 * 1024;
    fprintf(stderr, "Warning: File too large, reading only first 500MB: %s\n",
            filepath);
  }
...
void trim_string(char *str) {
  ...
  // Ищем первый не-пробельный символ с начала
  while (isspace((unsigned char)str[begin])) {
    begin++;
  }

  // Ищем первый не-пробельный символ с конца
  while (end >= begin && isspace((unsigned char)str[end])) {
    end--;
  }

  // Сдвигаем все символы в начало массива
  int i;
  for (i = begin; i <= end; i++) {
    str[i - begin] = str[i];
  }
...
int is_already_running(const char *lockfile_path, Config *config) {
  ...
  void remove_lock_file(const char *lockfile_path, Config *config) {
    if (!lockfile_path) {
      // проверка config перед использованием
      if (config) {
        log_message(config, "ERROR", "NULL lockfile path");
      }
      return;
...
// RISC-V архитектура
  ...
  char *calculate_file_hash(const char *filepath, const char *algorithm) {
    if (!filepath || !algorithm) {
      fprintf(stderr, "calculate_file_hash: NULL parameters\n");
      return NULL;
    }
    ...
    if (strcasecmp(algorithm, "md5") == 0) {
      ...
      // По умолчанию SHA256
      fprintf(stderr, "Unknown algorithm '%s', using SHA256\n", algorithm);
      md_algorithm = EVP_sha256();
      hash_len = SHA256_DIGEST_LENGTH;
    }
    ...
    // Для больших файлов (>100MB) используем частичное хеширование
    const long CHUNK_SIZE = 10 * 1024 * 1024; // 10 MB
    const size_t BUFFER_SIZE = 1048576;       // 1 МБ
    ...
    if (file_size > 100 * 1024 * 1024) { // Файл > 100MB
      // Хешируем начало файла
      long remaining = CHUNK_SIZE;
      while (remaining > 0 &&
             (bytes_read =
                  fread(buffer, 1,
                        (remaining < (long)BUFFER_SIZE) ? (size_t)remaining
                                                        : BUFFER_SIZE,
                        file)) > 0) {
      ...
      // Хешируем конец файла
      if (fseek(file, -CHUNK_SIZE, SEEK_END) != 0) {
        // Если не удалось перейти в конец, хешируем весь файл
        fprintf(stderr,
                "Warning: Cannot seek to end of large file, hashing entire "
                "file: %s\n",
                filepath);
        rewind(file);
        total_read = 0;
    ...
    char *hash_str = malloc(hash_len_out * 2 + 1);
    if (!hash_str) {
      free(hash);
      return NULL;
    }

    for (unsigned int i = 0; i < hash_len_out; i++) {
      sprintf(hash_str + (i * 2), "%02x", hash[i]);
    }
    hash_str[hash_len_out * 2] = '\0';
  ...
  char *calculate_buffer_hash(const unsigned char *buffer, size_t len,
    ...
    // Если len == 0, возвращаем хеш пустой строки
    if (len == 0) {
      fprintf(stderr, "calculate_buffer_hash: len=0, returning empty hash\n");
      // Возвращаем хеш для пустой строки (md5("") =
      // d41d8cd98f00b204e9800998ecf8427e)
      return safe_strdup("d41d8cd98f00b204e9800998ecf8427e");
    }

    if (!buffer || !algorithm) {
      fprintf(stderr,
              "calculate_buffer_hash: Invalid parameters (buffer=%p, "
              "algorithm=%s)\n",
              (void *)buffer, algorithm ? algorithm : "NULL");
      return NULL;
    }

    if (!buffer || len == 0 || !algorithm) {
      fprintf(stderr,
              "calculate_buffer_hash: Invalid parameters (buffer=%p, len=%zu, "
              "algorithm=%s)\n",
              (void *)buffer, len, algorithm ? algorithm : "NULL");
      return NULL;
    }
  ...
  /**
   * Проверяет, является ли строка валидным UTF-8
   * @param str Строка для проверки
   * @return 1 если валидный UTF-8, 0 если нет
   */
  int is_valid_utf8_string(const char *str) {
    if (!str)
      return 1; // NULL считается валидным

    const unsigned char *bytes = (const unsigned char *)str;
    while (*bytes) {
      // ASCII символы (0x00-0x7F)
      if ((*bytes & 0x80) == 0) {
        bytes += 1;
      }
   ...
   * @return Новая строка с заменой некорректных символов на '?'
   */
  char *sanitize_utf8_string(const char *str) {
    if (!str)
      return NULL;
    if (is_valid_utf8_string(str))
      return safe_strdup(str);
    ...
    while (*bytes && j < len) {
      ...
      // 2-байтовый UTF-8
      else if ((*bytes & 0xE0) == 0xC0 && bytes[1]) {
        if ((bytes[1] & 0xC0) == 0x80) {
          // Валидная 2-байтовая последовательность
          clean[j++] = *bytes++;
          clean[j++] = *bytes++;
        } else {
      ...
      // 3-байтовый UTF-8
      else if ((*bytes & 0xF0) == 0xE0 && bytes[1] && bytes[2]) {
        if ((bytes[1] & 0xC0) == 0x80 && (bytes[2] & 0xC0) == 0x80) {
          clean[j++] = *bytes++;
          clean[j++] = *bytes++;
          clean[j++] = *bytes++;
      ...
      // 4-байтовый UTF-8
      else if ((*bytes & 0xF8) == 0xF0 && bytes[1] && bytes[2] && bytes[3]) {
        if ((bytes[1] & 0xC0) == 0x80 && (bytes[2] & 0xC0) == 0x80 &&
            (bytes[3] & 0xC0) == 0x80) {
          clean[j++] = *bytes++;
          clean[j++] = *bytes++;
